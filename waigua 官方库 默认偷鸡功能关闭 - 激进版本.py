"""
--------普达特信号群跟单机器人-----
V2.0.1支持本地运行，支持多个交易所跟单（币安、欧意、芝麻、bitget、bitmark、huobi 支持定制） exe可执行文件，json配置好API，即可双击运行。支持第三方信号转发，比如群1信号毫秒级别转发到群2. 收费100U
v2026.3.18更新
        ...
        try:
            ...
            while long_positions and conservative_mode:
                ...
                if long_quantity > 20 and huoli >= 140:  # 持仓大于100且获利大于等于110（保本）
                    await handle_partial_closure(symbol, long_positions, long_quantity, huoli, real_time_price, "多",
                                                 0.8)
        ...
        try:
            ...
            while short_positions and conservative_mode:
                ...
                if short_quantity > 20 and huoli >= 140:  # 持仓大于100且获利大于等于110
                    await handle_partial_closure(symbol, short_positions, short_quantity, huoli, real_time_price, "空",
                                                 0.8)
                    await asyncio.sleep(1)  # 每轮条件满足，执行后歇1秒，防止仓位未更新多条件触发
...
async def handle_partial_closure(symbol, positions, quantity, huoli, real_time_price, action, fraction):
    ...
    while retry_count <= max_retries:
        try:
            ...
            positions, remaining_quantity = update_position_for_partial_closure(positions,
    ...
    while retry_count <= max_retries:
        try:
            ...
            if result:
                ...
                main_logger.info(
                    f"补仓开{action}成功: {result['amount']} 张，当前价格: {real_time_price},当前亏损{loss}")
...
def parse_new_signal(text):
    ...
    # 兼容消息前面可能带了机器人名称、空行等情况
    has_new_signal_flag = any("AI" in line for line in lines)
    if not has_new_signal_flag:
        return None
    ...
    for line in lines:
        ...
        # 2. 解析交易品种
        elif line.startswith("交易品种:") or line.startswith("交易品种："):
            base = re.split(r"[:：]", line, maxsplit=1)[1].strip()
        ...
        # 3. 解析开仓数量
        elif line.startswith("开空数量:") or line.startswith("开空数量：") \
                or line.startswith("开多数量:") or line.startswith("开多数量："):
            try:
                open_lot = float(re.split(r"[:：]", line, maxsplit=1)[1].strip())
            except Exception:
                open_lot = None

        # 4. 解析平仓数量
        elif line.startswith("平仓数量:") or line.startswith("平仓数量："):
            try:
                close_lot = float(re.split(r"[:：]", line, maxsplit=1)[1].strip())
            except Exception:
                close_lot = None
...
async def handle_message(message, client):
    ...
    try:
        ...
        if new_signal:
            ...
            if not match:
                main_logger.warning(f"未识别的指令: {repr(message_text)}")
                await client.send_message(
                    CHANNEL_ID,
                    f"未识别的指令:\n{message_text}\n\n"
                    f"当前支持格式：\n"
                    f"1. 新格式：激进版AI 1.0 / 市价开多 / 市价开空 / 多单平仓 / 空单平仓\n"
                    f"2. 旧格式：[开多] 数量:1 市场:BTC-USDT"
                )
                return
